...
// Per-locale Open Graph / Twitter image for /result, 1200×630.
//
// GENERIC + NAME-FREE by design: a shared /result link must NEVER preview a
// child's name or result (children's-data minimisation). It shows only on-brand
// "I finished the IqUp Brain Games" copy. Rendered with the Cyrillic-capable
// Rubik font so the Macedonian image never ships tofu glyphs. Hex values are
// inlined (satori can't resolve CSS theme tokens) — keep in sync with the tokens.
...
export default async function ResultOpengraphImage({
  ...
  const t = await getTranslations({locale, namespace: 'Result.og'});
