...
| `src/messages/mk.json` | Macedonian UI strings (Meta, Landing, Test, Gate, Result, Email, Consent, Privacy, Trial, Assessment, Form, Results, **Certificate** [v2 certificate chrome + per-index `strengthLine` + OG copy, Phase 3.11] namespaces; default locale; MK provisional). |
| `src/messages/en.json` | English UI strings (mirror of mk.json incl. **Certificate**). |
| `src/messages/messages.test.ts` | Vitest i18n parity suite (identical mk↔en key sets + matching `{placeholders}` + no empty strings; required-key + forbidden-token/no-digit lists incl. the `Email`, `Consent`, `Privacy`, `Results` + **`Certificate`** namespaces). |
...
| `src/app/globals.css` | Tailwind v4 + the 1.03 brand tokens (palette, status, strengths, radii, shadows, motion) via `@theme inline`; reduced-motion reset; light-only (no `.dark`). **+ v2 brand primitives (Phase 3.01):** official palette (`--iq-*`, `--index-*`), `--font-brand` Montserrat, `rounded-card/-lg/-badge`, spacing + `--tap-min`. **+ v2 semantic layer + `.iq-results` styles (Phase 3.09):** the `--ix-*` ramps, `--action*`, `--band-*`, `--surface-2`, `--ink-head/-muted`, `--neutral`, `--line*`, `--r-*`, `--tap-comfort`, `--elev-*`, `--focus*`, `--dur-fast` (lifted from `tokens-v2.css`; `--ix-*`→`--iq-*`) + the scoped results-screen CSS (all additive; v1 tokens kept). **+ the scoped `.iqc-*` certificate-panel CSS (Phase 3.11):** intro, the opt-in on-device name field, the scaled preview, the download/share actions + the error-status variant (uses only DEFINED tokens; the artwork itself uses inline styles for html-to-image fidelity). |
...
| `src/components/report/ReportFlow.tsx` | **Modified (3.11)** — the form + results client island: reads the persisted run + `iqup.leadContext.v2`, recomputes the profile client-side, captures parent + 3 consents + honeypot, writes both stores, then reveals `ResultsScreen` — now injecting `<CertificatePanel>` into the `// SEAM (3.11)` certificate slot (also passes `report` to `submitAssessment` for the 3.10 email). Refresh re-reveals; `// SEAM (3.10)/(3.12)` left. WCAG 2.2 AA. |
| `src/components/report/ResultsScreen.tsx` | **Modified (3.11)** — Surface A results screen (pure/presentational; props `report`/`copy`/`locale`/`bookingUrl` + the new optional `certificate?: ReactNode` slot). Header + pentagon + five index cards + top strength + what-we-noticed + emailed strip + demo CTA + **certificate (the 3.11 slot, else the entry affordance)** + disclaimer. No magnitude; deterministic; node-renderable. |
...
| `src/components/report/CertificateArt.tsx` | **New (3.11)** — the pure, props-driven 1080×1350 (4:5) certificate artwork (`forwardRef` captured node; inline styles for faithful html-to-image raster). IQ UP! wordmark (structural) + "Explorer" reward + the Bibi placeholder (`BIBI_CERT_ART` drop-in) + top strength (name pill + warm `strengthLine`) + per-child accent from the top-strength hue + optional on-device name + month/year. No score/band; deterministic. |
| `src/components/report/CertificatePanel.tsx` | **New (3.11)** — `'use client'` wrapper: the panel intro + the opt-in (off-by-default) **on-device-only** name field + the responsive scaled preview + Download (`html-to-image`→PNG, Montserrat-only font embed + timeout→`skipFonts` fallback) + Share (Web Share file-share, generic metadata; copy-link → PII-free landing URL). The name never leaves the device. |
| `src/components/report/cert-model.ts` | **New (3.11)** — pure cert logic: `certAccent(topIndex)` (per-child accent tokens from `INDEX_META` hue), `certMonthYear(date, locale)` (static month table, no clock), the deterministic confetti table + hues. |
| `src/components/report/certificate-copy.ts` | **New (3.11)** — the `CertificateCopy` chrome contract (resolved server-side from the `Certificate` namespace; `strengthLine` keyed by `IndexId`; the OG copy). |
| `src/components/report/certificate.test.tsx` | **New (3.11)** — renders the artwork via `react-dom/server`: forbidden-token scan (5 indices × 2 locales, wordmark-stripped) + no-stray-digit (only the keepsake date) + determinism + name-off-by-default + name-only-in-image (never an attribute) + the no-leak source scan (no store/network/analytics sink) + the OG name-free-by-construction scan. |
| `src/components/report/cert-accent.test.ts` | **New (3.11)** — AA contrast across every accent (ink-on-tint + ink-on-white ≥4.5, white-on-violet ≥4.5, the constant card text), the deterministic accent map, `certMonthYear`, and a guard proving the raw accent solid would fail AA behind text. |
| `src/components/report/copy.ts` | The `FormCopy` contract (resolved server-side, threaded to the island). |
| `src/components/report/index.ts` | **Modified (3.11)** — module barrel (`ReportFlow`, `ResultsScreen`, `IdentityPentagon`, **`CertificatePanel`, `CertificateArt`**, `FormCopy`, `ResultsCopy`, **`CertificateCopy`**). |
| `src/app/[locale]/report/page.tsx` | **Modified (3.11)** — SSG shell resolving the `Form` + `Results` **+ `Certificate`** namespaces, mounting `ReportFlow` (container `max-w-[1080px]`); `robots: noindex`. The `// HANDOFF (3.06)` destination. |
| `src/app/[locale]/report/opengraph-image.tsx` | **New (3.11)** — generic, **name-free** per-locale `/report` (results/share) OG image (`next/og`, 1200×630, Cyrillic-safe Montserrat from the committed `src/lib/pdf/fonts/` TTFs; v2 violet/brand tokens). Takes only `{locale}`. |
